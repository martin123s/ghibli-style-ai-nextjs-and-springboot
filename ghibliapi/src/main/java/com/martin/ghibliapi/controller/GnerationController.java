package com.martin.ghibliapi.controller;

import com.martin.ghibliapi.dto.TextGenerationRequestDTO;
import com.martin.ghibliapi.service.GhibliArtService;
...
public class GnerationController {

    private final GhibliArtService ghibliArtService;

    @PostMapping(value="/generate-from-image", produces = MediaType.IMAGE_PNG_VALUE)
    public ResponseEntity<byte[]> generateGhibliArtFromImage(@RequestParam("image")MultipartFile image,
                                                    @RequestParam("prompt")String prompt){
        try{
            byte[] imageBytes = ghibliArtService.createGhibliArtFromImage(image, prompt);
